...
vec3 BlurScene(in vec2 uvPos, in vec2 pixelFrac)
{
  ...
  colorVals[width] = texture(DiffuseColor_TexU_Texture, uvPos).rgb;
  
  for (int i = 0; i < width; ++i)
  {    
    vec3 depthLeft  = texture(DiffuseColor_TexU_Texture, uvPos-vec2((width-i)*pixelFrac.x,0)*BlurStrength).rgb;
    vec3 depthRight = texture(DiffuseColor_TexU_Texture, uvPos+vec2((width-i)*pixelFrac.x,0)*BlurStrength).rgb;
  ...
  for (int i = 0; i < width; ++i)
  {    
    vec3 depthTop  = texture(DiffuseColor_TexU_Texture, uvPos-vec2(0, (width-i)*pixelFrac.y)*BlurStrength).rgb;
    vec3 depthDown = texture(DiffuseColor_TexU_Texture, uvPos+vec2(0, (width-i)*pixelFrac.y)*BlurStrength).rgb;
...
void main()
{
  ...
  if (DebugOutputIndex == DEBUG_OUTPUT_COMBINED)
  {
    ...
    if(EnableBlur != 0)//enable blur
      ...
      pixelMatColor = texture(DiffuseColor_TexU_Texture, vec2((gl_FragCoord.xy) * pixelFrac)).xyz;
    //////////////////////////////////////////////////////////////////////////////////////////////////
